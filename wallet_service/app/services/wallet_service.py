...
class WalletService:
    ...
    def create_wallet(self, payload: WalletCreate, current_user_id: int):
        user = self.users.get_by_id(current_user_id)
        if not user:
            raise NotFoundError("User not found.")

        if self.wallets.get_by_user_id(current_user_id):
            raise ConflictError("User already has a wallet.")

        try:
            wallet = self.wallets.create(current_user_id)
            self.db.commit()
    ...
    def get_balance(self, wallet_id: int, current_user_id: int):
        wallet = self._get_owned_wallet_or_forbidden(wallet_id, current_user_id)
        return wallet

    def credit(self, wallet_id: int, payload: MoneyOperation, current_user_id: int):
    ...
    def debit(self, wallet_id: int, payload: MoneyOperation, current_user_id: int):
        return self._apply_transaction(
            wallet_id=wallet_id,
            current_user_id=current_user_id,
    ...
    def get_transactions(self, wallet_id: int, current_user_id: int):
        self._get_owned_wallet_or_forbidden(wallet_id, current_user_id)
    ...
    def _apply_transaction(
        ...
        try:
            with self.db.begin():
                ...
                wallet = self.wallets.get_by_id_and_user_id_for_update(
                    wallet_id=wallet_id,
                    user_id=current_user_id,
                )
                if not wallet:
                    self._raise_wallet_access_error(wallet_id)
            ...
            return wallet, entry
        except (NotFoundError, ForbiddenError, InsufficientFundsError, ConflictError):
            raise
        except IntegrityError as exc:
            self.db.rollback()
            raise ConflictError("Idempotency key already exists.") from exc

    def _get_owned_wallet_or_forbidden(
        self,
        wallet_id: int,
        current_user_id: int,
    ) -> Wallet:
        wallet = self.wallets.get_by_id_and_user_id(wallet_id, current_user_id)
        if wallet:
            return wallet
        self._raise_wallet_access_error(wallet_id)

    def _raise_wallet_access_error(self, wallet_id: int) -> None:
        if self.wallets.get_by_id(wallet_id):
            raise ForbiddenError("You are not allowed to access this wallet.")
