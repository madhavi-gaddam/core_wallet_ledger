from fastapi import APIRouter, Depends, status
from sqlalchemy.orm import Session

from app.auth.dependencies import get_current_user
from app.database import get_db
from app.models.user import User
from app.schemas.ledger_entry import LedgerEntryRead
from app.schemas.wallet import (
    BalanceRead,
    MoneyOperation,
    WalletCreate,
    WalletRead,
    WalletTransactionResponse,
)
from app.services.wallet_service import WalletService

router = APIRouter(prefix="/wallets", tags=["Wallets"])


@router.post("", response_model=WalletRead, status_code=status.HTTP_201_CREATED)
def create_wallet(
    payload: WalletCreate,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    return WalletService(db).create_wallet(payload, current_user.id)


@router.post("/{wallet_id}/credit", response_model=WalletTransactionResponse)
def credit_wallet(
    wallet_id: int,
    payload: MoneyOperation,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    wallet, entry = WalletService(db).credit(wallet_id, payload, current_user.id)
    return WalletTransactionResponse(
        wallet_id=wallet.id,
        balance=entry.balance_after_transaction,
        updated_balance=entry.balance_after_transaction,
        transaction_id=entry.id,
        ledger_entry_id=entry.id,
    )


@router.post("/{wallet_id}/debit", response_model=WalletTransactionResponse)
def debit_wallet(
    wallet_id: int,
    payload: MoneyOperation,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    wallet, entry = WalletService(db).debit(wallet_id, payload, current_user.id)
    return WalletTransactionResponse(
        wallet_id=wallet.id,
        balance=entry.balance_after_transaction,
        updated_balance=entry.balance_after_transaction,
        transaction_id=entry.id,
        ledger_entry_id=entry.id,
    )


@router.get("/{wallet_id}/balance", response_model=BalanceRead)
def get_wallet_balance(
    wallet_id: int,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    wallet = WalletService(db).get_balance(wallet_id, current_user.id)
    return BalanceRead(wallet_id=wallet.id, balance=wallet.balance)


@router.get("/{wallet_id}/transactions", response_model=list[LedgerEntryRead])
def get_wallet_transactions(
    wallet_id: int,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    return WalletService(db).get_transactions(wallet_id, current_user.id)
